...
ohlcv_histories_normalised = np.array([train_data['Adj Close'][i  : i + history_points].copy() for i in range(DAYS - history_points)])
next_day_open_values_normalised = np.array([train_data['Adj Close'][i + history_points].copy() for i in range(DAYS - history_points)])
next_day_open_values_normalised = np.expand_dims(next_day_open_values_normalised, -1)
 
 
next_day_open_values = np.array([train_data['Adj Close'][i + history_points].copy() for i in range(DAYS - history_points)])
...
print(test_data['Adj Close'][0:DATA - DAYS + 1])
  
ohlcv_histories_normalised_test = np.array([test_data['Adj Close'][i  : i + history_points].copy() for i in range(DATA - DAYS - history_points)])
next_day_open_values_normalised_test = np.array([test_data['Adj Close'][i + history_points].copy() for i in range(DAYS, DATA - history_points)])
next_day_open_values_normalised_test = np.expand_dims(next_day_open_values_normalised_test, -1)
 
 
next_day_open_values_test = np.array([test_data['Adj Close'][i + history_points].copy() for i in range(DAYS, DATA - history_points)])
...
exponential_moving_average_normalised = np.array(exponential_moving_average)
...
exponential_moving_test_normalised = np.array(exponential_moving_test)
...
# model.fit(x=[ohlcv_train, exponential_moving_average_normalised], y=y_train, batch_size=32, epochs=30, shuffle=True, validation_split = 0.1) #callbacks = [history]
